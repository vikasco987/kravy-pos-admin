...
export default function UserDetailPage() {
  ...
  const fetchUser = async () => {
    try {
      const [userRes, rolesRes] = await Promise.all([
        fetch(`http://localhost:15432/api/admin/users/${id}`),
        fetch('http://localhost:15432/api/admin/roles')
      ]);
  ...
  const handleAddIdentifier = async (type: 'email' | 'phone') => {
    ...
    try {
      ...
      if (res.ok) {
        playSound('success');
        alert(`${type === 'email' ? 'Email' : 'Phone'} added`);
    ...
    confirmAction(
      ...
      async () => {
        ...
        try {
          ...
          if (res.ok) {
            playSound('success');
            alert("User logged out everywhere");
            fetchUser();
          } else {
            playSound('error');
            const err = await res.json();
  ...
  const revertToRoleDefaults = async () => {
    ...
    try {
      ...
      if (res.ok) {
        playSound('pop');
        alert("Permissions reverted to default");
        setShowPermissionsModal(false);
        fetchUser();
      } else {
  ...
  const handleImpersonate = async () => {
    ...
    try {
      const res = await fetch("http://localhost:15432/api/admin/users/impersonate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
