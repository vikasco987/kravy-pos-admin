...
export default function ItemModal({ item, addonGroups = [], onSave, onClose, categories = [] }: any) {
    ...
    const handleSaveQuickZone = async () => {
      ...
      if (!name) return;
      try {
        const res = await fetch("http://localhost:15432/api/profile/zones", {
          method: "POST",
    ...
    async function handleQuickAddonSave(data: any) {
        const res = await fetch(`http://localhost:15432/api/menu-editor/addon-groups`, {
    ...
    async function handleQuickAddonDelete(id: string) {
        ...
        const res = await fetch(`http://localhost:15432/api/menu-editor/addon-groups?id=${id}`, { method: 'DELETE' });
        if (res.ok) {
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        setUploading(true);
        const res = await fetch("http://localhost:15432/api/upload", { method: "POST", body: formData });
        const data = await res.json();
              ...
              <div className="space-y-6 flex flex-col items-center pt-2">
                 <div className="w-full h-56 rounded-[2rem] border-2 border-dashed border-[var(--kravy-border)] bg-[var(--kravy-bg)] relative overflow-hidden flex items-center justify-center group">
                    {uploading ? (
                      ...
                      <>
                        <img src={local.imageUrl} alt="Preview" fill className="object-cover" />
